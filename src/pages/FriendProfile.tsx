...
const FriendProfile = () => {
    ...
    // Fetch friend's profile data (user + their habits)
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const { data } = await api.get(`/profile/${userId}`);
                setProfile(data); // Set fetched data
            } catch (error) {
            ...
            <div className="bg-white dark:bg-dark-card p-8 rounded-lg shadow-md mb-8">
                <div className="flex items-center space-x-6 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400 dark:text-dark-subtext mt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {habits.map(habit => (
                    <HabitCard
                        key={habit._id}
                        habit={habit}
                        onToggle={() => {}}         // Disabled actions for friend view
                        onDelete={() => {}}
                        onSetReminder={() => {}}
                        isFriendView={true}         // Flag to adjust card behavior for friend mode
                    />
                ))}
                {habits.length === 0 && (
                    <p className="text-gray-500 dark:text-dark-subtext">
                        This user hasn't added any habits yet.
                    </p>
                )}
            </div>
        </div>
