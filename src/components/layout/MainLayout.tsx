import { useState, ReactNode } from 'react'; // Importing hooks and types
import { NavLink } from 'react-router-dom'; // For navigation links
import { useTheme } from '../../contexts/ThemeContext'; // Custom theme context hook
import { useAuth } from '../../hooks/useAuth'; // Custom auth hook for user info and logout
...
// MainLayout component wraps the app with header and main layout
const MainLayout = ({ children }: { children: ReactNode }) => {
  const { theme, setTheme } = useTheme(); // Access theme and setter
  const { user, logout } = useAuth(); // Access user info and logout method
      ...
      <header className="bg-white dark:bg-dark-card shadow-sm sticky top-0 z-10">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <button
                ...
                className="p-2 rounded-full text-gray-500 dark:text-dark-subtext hover:bg-gray-100 dark:hover:bg-gray-700"
              >
              ...
              <button
                onClick={logout}
                className="flex items-center space-x-2 text-sm text-gray-500 dark:text-dark-subtext hover:text-gray-700 dark:hover:text-dark-text"
              >
                <SignOutIcon />
            ...
            {/* Mobile menu toggle (hamburger and theme) */}
            <div className="md:hidden flex items-center">
              <button
                onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
                className="p-2 rounded-full text-gray-500 dark:text-dark-subtext"
              >
                {theme === 'dark' ? <SunIcon /> : <MoonIcon />}
              </button>
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="p-2 rounded-md text-gray-500 dark:text-dark-subtext"
              >
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white dark:bg-dark-card border-t border-gray-200 dark:border-gray-700">
            ...
            {/* Mobile user and logout section */}
            <div className="pt-4 pb-3 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="mt-3 space-y-1">
                <button
                  ...
                  className="w-full text-left block px-4 py-2 text-base font-medium text-gray-500 hover:text-gray-800 hover:bg-gray-100 dark:text-dark-subtext dark:hover:text-dark-text dark:hover:bg-gray-700"
                >
                  Sign Out
                </button>
              </div>
...
export default MainLayout; // Exporting the layout component
